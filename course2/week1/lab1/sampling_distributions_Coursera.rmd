...
```{r}
# Insert code here to display the types of these two population parameters.
```
...
<li> 50\% of houses in Ames are smaller than 1,499.69 square feet. <span style="color: #f00;">FALSE: the median is 1442</span> </li>
...
<li> Sample size of 1000. <span style="color: #f00;">This one. As the sample size increases both <i>accuracy</i> and <i>precision</i> increase.</span></li>
</ol>
...
```{r loop}
# rep_sample_n will take as many samples as you wish, and group them
# by the new column 'replicate'. The resulting table is otherwise
# structured the same as the source table 'ames'. We pipe the result
# to 'summarise' which calculates the mean for every sample. Not
# known: how 'summarise' knows not to give the mean for all 'area'
# instances, but instead group them by 'replicate'. Magic column name
# perhaps?  sample_means50 is of type 'list' despite the text here
# claiming it is a vector but see:
# https://stackoverflow.com/questions/8594814/what-are-the-differences-between-r-vector-and-r-list-data-types.
sample_means50 <- ames %>%
...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
# what do they mean by 'elements' I wonder?
dim(sample_means50)

```
<div style="background-color:#00ff00;">
sample_means50 has 15,000 rows with two variables each. The sampling distribution is unimodal with a slight right skew. (I note here that in the forums someone else asks "what is an element?") I wager this list has 15,000 elements.
</div>
...
<li> 0 </li>
<li> 3 </li>
<li> 25 <span style="color: #f00;">This is the answer</span></li>
<li> 100 </li>
<li> 5,000 </li>
</ol>
...
<ol>
<li> Each element represents a mean square footage from a simple random sample of 10 houses.  <span style="color: #f00;">This is the answer</span></li>
...
<ol>
<li> decreases <span style="color: #f00;">This is the answer</span></li>
<li> increases </li>
...
samp2 %>% summarise(xbar = mean(price))
```
...
ggplot(data = sample_means50, aes(x = x_bar)) +
  geom_histogram(binwidth = 1000) + labs(title="sample size 50: home prices")
```
...
ggplot(data = sample_means150, aes(x = x_bar)) +
  geom_histogram(binwidth = 500) + labs(title="sample size 150: home prices")
```
...
samp3 <- ames %>%
  sample_n(size = 15)
summarise(samp3, x_bar = mean(price))
```
...
ggplot(data = sample_means15, aes(x = x_bar)) +
  geom_histogram(binwidth = 1000) + labs(title="sample size 15, 2,000 samples: home prices")

ggplot(data = ames, aes(x = price)) +
  geom_histogram(binwidth = 10000) + labs(title="All home prices")
```
...
ggplot(data = sample_means150, aes(x = x_bar)) +
  geom_histogram(binwidth = 750) + labs(title="sample size 150, 2,000 samples: home prices")

```

6. Which of the following is false? 
<ol>
<li> The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`).<span style="color: #f00;">This is the answer</span></li>
...
# Uh, question 6 does not want us to run code. I'm guessing this block is copypasta.
```
