...
<li class="answer"> <b>The births</b></li>
</ol>
...
<li class="answer"> Both distributions are extremely right skewed. </li>
<li> The IQRs of the distributions are roughly equal. </li>
</ol>
```{r habit-weight-box}
# side by side box plots of habit and weight (habit being t/f)
...
<div class="answer">
<ul>
<li>It is a random sample because it's in our lab and teacher wouldn't mislead us.</li>
<li>The data size is sufficiently large (greater than 30)</li>
<li>We have independence. The weight gain of a mother does not depend on the weight gain of any other mother.</li>
</ul>
</div>
...
<li class="answer"> $H_0: \mu_{smoking} = \mu_{non-smoking}$; $H_A: \mu_{smoking} \ne \mu_{non-smoking}$ </li>
...
<li class="answer"> We are 95% confident that babies born to nonsmoker mothers are on average 0.05 to 0.58 pounds heavier at birth than babies born to smoker mothers. </li>
</ol>
```{r habit-weight-ci}
...
<li>(6.9779 , 7.2241)</li>
<li class="answer">(38.0952 , 38.5742)</li>
</ol>
```{r weeks-ci-99}
...
<div class="answer">They appear identical.</div>
...
```{r cutoff-mature-too}
younglings <- nc %>% filter(mature == "younger mom")
summary(younglings$mage)

elders <- nc %>% filter(mature == "mature mom")
summary(elders$mage)
```
...
<div class="answer">
We see that "younger mom" is up to 34 years old, and 35 is the beginning of the ages of the "mature mom."
</div>

## My bonus analysis: not so helpful though...

```{r cutoff-mature}
IQR(younglings$mage, na.rm = TRUE)

IQR(elders$mage, na.rm = TRUE)
```

## My bonus analysis two...
...
<div>
Is there a statistical difference in premature births and the maturity of the mother? We will look at the categorical variable `premie` and the numerical variable `mage`.

We already know that:
<ul>
<li>We are dealing with a random sample</li>
<li>Our data size is sufficiently large</li>
<li>Each measurement is independent</li>
</ul>

We see that we cannot reject the null hypothesis here because we wind up with a p_value of 0.9224.

</div>

```{r pick-your-own}
inference(y = fage, x = premie, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
```
