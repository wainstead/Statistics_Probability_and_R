...
on these samples in this lab, which means you should set a seed on top of your lab. If this concept is new to you, review the previous lab and ask your TA.
...
# "typical" is kind of vague, I guess. "typical" probably means "most parameters are close to the average" where applicable, but there could be several kinds of "typical" homes (mansions versus shotguns versus bungalos versus midmods)
samp %>% summarise(xbar = mean(area), sample_med = median(area), 
...
#### False, because we don't know anything about the other samples. How large were the sample sizes?
#### Or it could be true because the question is kind of vague. "My sample is similar but not likely exactly the same" sounds quite true to me.
...
#### C. We know the <i>sampling</i> distribution will be nearly normal. The sample distribution will refect the shape of the population distribution if n is large enough.
...
#### B, which has the most precision
...
samp %>% summarise(lower = mean(area) - qnorm(0.975) * (sd(area) / sqrt(n)),
...
#### Something of a guess, but D. I am visualizing the 95% confidence interval as the area under the normal distribution that captures 95% of the total area under the curve, while the margin of error describes a lower bound and upper bound.
...
```{r plot-99-perc-cis}
# type your code for the Exercise here, and Knit
# It would be: (1 - CI) / 2, which for 95% would be (1 - 0.95) / 2 == 1.958 (1.96)
# so for a 99% CI we would figure it: (1 - 0.99) / 2 == 0.005 and we add the 0.99 back in for 0.995
qnorm(0.995)
```
