...
<li> histogram </li> 
<li> box plot </li> 
<li class="answer"> scatterplot </li> 
<li> bar plot </li>
</ol>
...
<ol>
<li class="answer"> linear </li> 
<li> negative </li> 
<li> horseshoe-shaped ($\cap$) </li>  
<li> u-shaped ($\cup$) </li>
</ol>
```{r delme, include=FALSE}
# copied from:
...
modl <- lm(mlb11$runs ~ mlb11$at_bats)
...
<div class="answer">not really, but I expect this lab thinks "yes"</div>
</div>
...
<li class="answer"> The relationship is positive, linear, and moderately strong. One of the potential outliers is a team with approximately 5520 at bats. </li>
...
<ol>
<li class="answer"> For each additional home run, the model predicts 1.83 more runs, on average. </li>
...
homerslm <- lm(formula = runs ~ homeruns, data = mlb11)
summary(homerslm)
```
<div class="answer">Formula: 415.2389 + (1.8345 x homeruns)</div>
...
# Note they ask for the answer for "at bats,", not homers.
...
6. Which of the following statements about the residual plot is <span class="answer">false</span>? 
<ol>
<li> The residuals appear to be randomly distributed around 0. </li> 
<li class="answer"> The residuals show a curved pattern. </li> 
...
7. Which of the following is <span class="answer">true</span>? 
<ol> 
...
<li class="answer"> The residuals are fairly symmetric, with only a slightly longer tail on the right, hence it would be appropriate to deem the the normal distribution of residuals condition met. </li>
</ol>
...
m1 <- lm(strikeouts ~ at_bats, data = mlb11)

ggplot(data = m1, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals (strikeouts, runs)")
...
<li> hits </li>
<li> wins </li>
<li class="answer"> batting average </li>
</ol>
```{r runs-vs-others}
# type your code for Question 9 here, and Knit
# there are seven traditionally used variables in the data set: at-bats, hits, home runs, batting average, strikeouts, stolen bases, and wins.

hitslm <- lm(runs ~ hits, data = mlb11)
summary(hitslm)

homerunslm <- lm(runs ~ homeruns, data = mlb11)
summary(homerunslm)

bat_avg_lm <- lm(runs ~ bat_avg, data = mlb11)
summary(bat_avg_lm)

ko_lm <- lm(runs ~ strikeouts, data = mlb11)
summary(ko_lm)

stolen_lm <- lm(runs ~ stolen_bases, data = mlb11)
summary(stolen_lm)
...
<ol>
<li class="answer"> on-base plus slugging (`new_obs`) </li>
<li> slugging percentage (`new_slug`) </li> 
<li> on-base percentage (`new_onbase`) </li>
</ol>
```{r runs-vs-new-vars}
# type your code for Question 10 here, and Knit
new_obs_lm <- lm(runs ~ new_obs, data = mlb11)
summary(new_obs_lm)

new_slug_lm <- lm(runs ~ new_slug, data = mlb11)
summary(new_slug_lm)

new_onbase_lm <- lm(runs ~ new_onbase, data = mlb11)
summary(new_onbase_lm)
...
```{r diag}
# type your code for the Exercise here, and Knit
ggplot(data = new_obs_lm, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals (on base and slugging, runs)")

ggplot(data = new_obs_lm, aes(sample = .resid)) +
  stat_qq()
```
