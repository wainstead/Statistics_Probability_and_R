...
<ol>
<li class="answer"> Observational study </li>
<li> Experiment </li>
</ol>
...
<li class="answer"> Yes, revise wording to "Is there an association between beauty and course evaluations?" </li>
...
<li class="answer"> The left skewness of the data suggests that the students are less likely to rate the professors highly. </li>
</ol>
```{r score-dist}
...
#qplot(evals$age, evals$bty_avg)
sp <- ggplot(evals, aes(x=age, y=bty_avg))
...
```{r summary-model}
# type your code for the Exercise here, and Knit
# sp <- ggplot(evals, aes(x=bty_avg, y=score))
# sp + geom_jitter() + stat_smooth(method=lm)

score_bty = lm(score ~ bty_avg, data = evals)
summary(score_bty)
```
<div class="answer">
y-hat = 3.88034 + 0.0664 * x
</div>

4. Average beauty score is a statistically significant predictor of evaluation score. 
<ol> 
<li class="answer"> True (because there is a clear (if weak) relationship even though the slope is small)</li>
<li> False</li>
</ol>

5. Use residual plots to evaluate whether the conditions of least squares regression are reasonable.  Which of the following statements is an <b>incorrect</b> analysis of the residual plots and conditions? 
<ol>
...
<li class="answer"> Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. (In fact there's a left skew).</li>
<li> Independent observations: Classes sampled randomly, no order effect. </li>
</ol>
```{r diag-score-bty_avg}
# type your code for the Question 5 here, and Knit
plot(score_bty$residuals)
hist(score_bty$residuals)
```
...
6. P-values and parameter estimates should only be trusted if the conditions for the regression are reasonable. Using diagnostic plots, we can conclude that the conditions for this model are reasonable. <span class="answer">The wording here is vague, and the correct answer was "true." It apparently asks about models in general, even though the wording is "for this model."</span>
<ol>
...
```{r summary-mlr-model}
# type your code for the Exercise here, and Knit
all_bty = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval + cls_did_eval + cls_students + cls_level + cls_profs + cls_credits + bty_f1lower + bty_f1upper + bty_f2upper + bty_m1lower + bty_m1upper + bty_m2upper + bty_avg + pic_outfit + pic_color, data = evals)
summary(all_bty)
```
...
<li class="answer"> Tenure Track, Tenured, Teaching </li>
...
<li class="answer"> 0.12 points higher than minority professors, all else held constant. </li> 
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_full_less_one <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_full_less_one)
